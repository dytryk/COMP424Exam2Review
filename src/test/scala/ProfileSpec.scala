...
class ProfileSpec2 extends AnyFlatSpec with Matchers {
  ...
  "profile" should "work for one sequence" in {
    LectureExcercises.profile(Seq("A")) should equal (Seq(Map('A' -> 1)))
    LectureExcercises.profile(Seq("ACG")) should equal (Seq(Map('A' -> 1), Map('C' -> 1), Map('G' -> 1)))
  }

  it should "work for multiple length-one sequences" in {
    LectureExcercises.profile(dnaTestGroups.head) should equal (Seq(Map('T'->3)))
    LectureExcercises.profile(dnaTestGroups(1)) should equal (Seq(Map('T'->1, 'A'->1, 'G'->2)))
  }

  it should "work for case 2" in {
    LectureExcercises.profile(dnaTestGroups(2)) should equal (Seq(
      Map('A'->2, 'C'->1),
      Map('G'->1, 'A'->2),
      Map('T'->2, 'C'->1)
    ))
  ...
  it should "work for case 3" in {
    LectureExcercises.profile(dnaTestGroups(3)) should equal (Seq(
      Map('A'->1, 'C'->2, 'G'->1),
      Map('G'->2, 'T'->1, 'C'->1),
      Map('G'->2, 'A'->1, 'C'->1),
      Map('A'->2, 'C'->1, 'G'->1),
  ...
  it should "work for case 4" in {
    LectureExcercises.profile(dnaTestGroups(4)) should equal (Seq(
      Map('A'->3, 'T'->2),
      Map('A'->3, 'T'->1, 'C'->1),
      Map('T'->3, 'A'->2),
      Map('G'->2, 'A'->1, 'C'->1, 'T'->1),
